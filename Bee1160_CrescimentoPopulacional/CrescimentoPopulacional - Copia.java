...
de duas cidades quaisquer (A e B), ela gostaria de saber quantos anos levará para que a cidade menor (sempre é
a cidade A) ultrapasse a cidade B em população. Claro que ela quer saber apenas para as cidades cuja taxa de
crescimento da cidade A é maior do que a taxa de crescimento da cidade B, portanto, previamente já separou para
você apenas os casos de teste que tem a taxa de crescimento maior para a cidade A. Sua tarefa é construir um
programa que apresente o tempo em anos para cada caso de teste.
...
caso de teste contém 4 números: dois inteiros PA e PB (100 ≤ PA < 1000000, PA < PB ≤ 1000000) indicando
respectivamente a população de A e B, e dois valores G1 e G2 (0.1 ≤ G1 ≤ 10.0, 0.0 ≤ G2 ≤ 10.0, G2 < G1) com um
digito após o ponto decimal cada, indicando respectivamente o crescimento populacional de A e B (em percentual).
...
public class CrescimentoPopulacional{
	...
	public static int PopulacaoAno(int populacao, double crescimento, int anos){
		double resultadoD = populacao;

		for (int i = 0; i < anos; i++){
			resultadoD += (populacao * crescimento) / 100;
		}

		int resultado = (int)resultadoD;
		return resultado;
	}
	...
	public static void main(String args[]){
		...
		for (int cont = 1; cont <= testes; cont++){
			...
			while (aAtual <= bAtual){
				aAtual = PopulacaoAno(pA, gA, tempo);
				bAtual = PopulacaoAno(pB, gB, tempo);


				if (tempo > 100){
					System.out.printf("Mais de 1 seculo.\n");
					break;
				} else if (aAtual > bAtual){
